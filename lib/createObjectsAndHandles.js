...
module.exports = (adsC, adapter) => {
    ...
    adapter.getStatesOf(adapter.namespace, 'plc', (err, res) => {
        ...
        for (let i = 0; i < res.length; i += 1) {
            ...
            const shortId = res[i]._id.replace(`${adapter.namespace}.`, '');
            ...
            if (!found) {
                ...
                adapter.delObject(shortId, error => {
                    ...
                    if (error) {
                        adapter.log.debug(
                            `Can not delete unnessecary Object or with wrong Datatyp (${shortId}). Error: ${error}`,
                        );
                    } else {
            ...
            adsC.releaseNotificationHandles(error => {
                ...
                (function createStatesAndSubscribe() {
                    ...
                    if (item.bytelength.name === 'BOOL') {
                        ...
                        ['TIME', 'TIME_OF_DAY', 'TOD', 'DATE', 'DATE_AND_TIME', 'DT'].includes(item.bytelength.name)
                    ) {
                            ...
                            adapter.setObjectNotExists(
                                ...
                                () => {
                                    ...
                                    adsC.notify(item, err => {
                                        if (err) {
                                            adapter.log.warn(
                                                `Subscription for: ${item.symname} occure an Error: ${err}`,
                                            );
                                        } else {
                                            adapter.log.debug(`Subscription for: ${item.symname} done.`);
                                        }

...
module.exports = (adapter, emitter) => {
    ...
    xml2js.parseString(
        ...
        (error, result) => {
            ...
            const vtables = adapter.config.targetVTable.toLowerCase().split(',');
            ...
            for (let i = 0; i < symbols.length; i += 1) {
                ...
                if (vtables.find(t => symName.toLowerCase().indexOf(t.trim()) === 1 && adapter.config.targetAmsPort === '901') != null) {
            ...
            try {
                adapter.config.symbolObjectList = createSymbolObjectList(adapter, dataTypes, filteredSymbols);
 ...
 */
function createSymbolObjectList(adapter, datatyps, symbols, datatypeIndex, prefix) {
        ...
        for (let i = 0; i < symbols.length; i += 1) {
            ...
            for (let j = 0; j < datatyps.length; j += 1) {
                ...
                if (datatyps[j].Name[0]._ === symbols[i].Type[0]._) {
                    symbolObjectList = symbolObjectList.concat(createSymbolObjectList(adapter, datatyps, symbols, j, `${tempObject.symname}.`));
    ...
    // When we found here some basic Datatypes -> Adding to Datapointarray with complete Path
        ...
        let found = false;
        let datatypetofind = datatyp[i].Type[0]._;
        const datatypearray = /^ARRAY \[(\d+)\.\.(\d+)\] OF (.+)$/.exec(datatypetofind);
        if (datatypearray != null) datatypetofind = datatypearray[3];
        for (let j = 0; j < datatyps.length; j += 1) {
            if (datatyps[j].Name[0]._ === datatypetofind) {
                if (datatypearray != null) {
                    for (let z = parseInt(datatypearray[1]); z <= parseInt(datatypearray[2]); z++)
                        symbolObjectList = symbolObjectList.concat(createSymbolObjectList(adapter, datatyps, symbols, j, `${prefix}${datatyp[i].Name[0]}[${z}].`));
                } else
                    symbolObjectList = symbolObjectList.concat(createSymbolObjectList(adapter, datatyps, symbols, j, `${prefix}${datatyp[i].Name[0]}.`));

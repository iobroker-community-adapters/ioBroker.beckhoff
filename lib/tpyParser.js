...
function createSymbolObjectList(datatyps, symbols, datatypeIndex, prefix) {
  const knownDatatypes = ['BOOL', 'BYTE', 'WORD', 'DWORD', 'SINT', 'USINT', 'INT', 'UINT', 'DINT', 'UDINT', 'REAL', 'LREAL', 'TIME', 'TIME_OF_DAY', 'TOD', 'DATE', 'DATE_AND_TIME', 'DT'];
  ...
  if (!prefix || typeof datatypeIndex === 'undefined') {
    for (let i = 0; i < symbols.length; i += 1) {
      ...
      if (symbols[i].Type[0]._.match('STRING\\(\\d+\\)')) {
        ...
        let check = symbols[i].Type[0]._.match('STRING\\((\\d+)\\)')
        if (check) {
          tempObject.bytelength = ads.string(parseInt(check[1]));
          symbolObjectList.push(JSON.parse(JSON.stringify(tempObject)));
          continue;
        }
      }
  ...
  for (let i = 0; i < datatyp.length; i += 1) {
    ...
    if (datatyp[i].Type[0]._.match('STRING\\(\\d+\\)')||knownDatatypes.includes(datatyp[i].Type[0]._)) {
      ...
      let check = datatyp[i].Type[0]._.match('STRING\\((\\d+)\\)')
      if (check) {
        tempObject.bytelength = ads.string(parseInt(check[1]));
        symbolObjectList.push(JSON.parse(JSON.stringify(tempObject)));
        continue;

...
module.exports = (adsC, adapter, emitter, state, id) => {
  ...
  adapter.getObject(id, (err, obj) => {
    ...
    if (obj.native.bytelength.name === 'STRING' && state.val.length > obj.native.bytelength.length - 1) {
      adapter.log.warn(`WorkerAdapterToPLC received a String that is longer than ${obj.native.bytelength.length - 1} Chars: ${symname} -> Cut to ${obj.native.bytelength.length - 1} Chars`);
